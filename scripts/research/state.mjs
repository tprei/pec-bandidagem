...
export function migrateLegacy(db) {
  ...
  for (const file of files) {
    ...
    for (const line of content.split("\n")) {
      ...
      if (value.responseId) db.prepare("INSERT OR IGNORE INTO provider_requests(request_key,sq,input_hash,provider,operation,payload_json,status,remote_id,response_json,headers_json,attempt_count,next_attempt_at,reserved_cost_usd,actual_cost_usd,error_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?, ?)").run(`legacy:${value.sq}:${value.responseId}`, value.sq, inputHash, "legacy", "synthesis", "{}", status, value.responseId, null, null, 0, timestamp, 0, null, null, timestamp, timestamp);
...
export function findOpenRemote(db, sq, inputHash, provider = "gemini") { return db.prepare("SELECT * FROM provider_requests WHERE sq = ? AND input_hash = ? AND provider = ? AND remote_id IS NOT NULL AND status IN ('synthesis_submitted','synthesis_polling','submitted') ORDER BY updated_at DESC LIMIT 1").get(sq, inputHash, provider); }
